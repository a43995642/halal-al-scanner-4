...
export const FloatingHeader: React.FC<FloatingHeaderProps> = ({
  ...
  return (
    <div className={`absolute top-0 left-0 right-0 z-20 p-4 pt-[calc(env(safe-area-inset-top)+10px)] flex justify-between items-start transform-gpu transition-all duration-500 ease-in-out ${(isFocusMode && !result) || isLoading ? '-translate-y-32 opacity-0' : 'translate-y-0 opacity-100'}`}>
      ...
      {setProductType && (
        <div className="absolute left-1/2 transform -translate-x-1/2 flex bg-black/30 backdrop-blur-md rounded-full p-1">
          <button onClick={() => setProductType('food')} className={`px-3 py-1 text-xs rounded-full transition ${productType === 'food' ? 'bg-emerald-500 text-white' : 'text-gray-300 hover:text-white'}`}>Food</button>
          <button onClick={() => setProductType('cosmetics')} className={`px-3 py-1 text-xs rounded-full transition ${productType === 'cosmetics' ? 'bg-emerald-500 text-white' : 'text-gray-300 hover:text-white'}`}>Cosmetics</button>
          <button onClick={() => setProductType('clothes')} className={`px-3 py-1 text-xs rounded-full transition ${productType === 'clothes' ? 'bg-emerald-500 text-white' : 'text-gray-300 hover:text-white'}`}>Clothes</button>
        </div>
      )}
